...
const DeleteFeatureConfirmationModal = ({ isOpen, onClose, feature }) => {
    const [isDeleting, setIsDeleting] = useState(false);
    const [error, setError] = useState("");

    const handleConfirmDelete = () => {
        if (isDeleting || !feature) return;

        setIsDeleting(true);
        setError("");

        router.delete(route('admin.catalogs.features.destroy', feature.id), {
            onSuccess: () => {
                // Successfully deleted
                onClose();
            },
            onError: () => {
                setError("Gagal menghapus fitur. Silakan coba lagi.");
                setIsDeleting(false);
            },
            onFinish: () => {
                setIsDeleting(false);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full text-gray-600">
                ...
                <p className="text-gray-600 mb-6 text-center">
                    Apakah Anda yakin ingin menghapus fitur <span className="font-semibold">{feature?.name}</span>? Tindakan ini tidak dapat dibatalkan.
                </p>
...
export default DeleteFeatureConfirmationModal;
